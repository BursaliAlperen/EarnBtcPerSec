// Firebase modüllerini import et (Eğer modüllerle çalışmıyorsan, aşağıdaki kısmı script tag ile klasik kullanım için değiştir)
...
// Klasik firebase SDK (script tag ile kullandığın varsayımıyla)
...
// BTC verisini her saniye Firestore'a kaydet
function startSavingBTCData(userId){
  if(btcInterval) clearInterval(btcInterval);

  btcInterval = setInterval(() => {
    const btcValue = getCurrentBTCValueSomehow();
    ...
    .then(docRef => {
      console.log("BTC verisi kaydedildi:", docRef.id);
    })
    .catch(err => {
...
// BTC değerini alacak örnek fonksiyon (burayı kendine göre değiştir)
function getCurrentBTCValueSomehow(){
  return (Math.random() * 50000 + 10000).toFixed(2); // Örnek rastgele fiyat
    }
const firebaseConfig = {
  apiKey: "AIzaSyD35fhSe9gdyESPY8-6Pmn2mQZlj94z2XE",
  authDomain: "earnbtcpersec.firebaseapp.com",
